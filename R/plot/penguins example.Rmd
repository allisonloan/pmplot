...
# Discrete x gets mapped internally; you can use midpoints like 1.0, 2.5, 3.0, etc.
# Here: bracket from the middle of bar 1 to halfway between bars 2 and 3
p_sig <- add_brackets_simple(
  p, side = "top",
  labels = "p < 0.05",
  start  = 1.0,   # middle of "A"
  end    = 2,   # halfway between "B" and "C"
  pos    = 8.2,   # y position in data units
  coord  = "data",
  label_offset = 0.15,
  label_size   = 10,
  fontface     = "bold"
...
p_sig2 <- add_brackets_simple(
  p, side = "top", coord = "data",
  labels = c("A vs B", "All groups"),
  start  = c(1.0, 1.0),
  end    = c(2.0, 3.0),
  pos    = c(8.2, 8.9),
  label_offset = c(0.12, 0.18),     # <- vectorized safely
  label_size   = c(10, 11),         # <- vectorized safely
  fontface     = c("bold", "plain") # <- vectorized safely
)
